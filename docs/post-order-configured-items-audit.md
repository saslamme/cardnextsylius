...
| `templates/bundles/SyliusShopBundle/order/show/content/header.html.twig` | öffentliche Bestell-/Zahlungsansicht, Positionsanzahl | beide Collections, `order.total` | Kunde | Ja | Ja | **B** – Sylius nutzte `totalQuantity`; jetzt werden normale plus konfigurierte Positionen gezählt. |
...
Collections (`items|length + configuredItems|length`), nicht verkaufte Stückmengen. Alle
