...
if (root) {
    ...
    const update = () => {
        ...
        root.querySelector('[data-advisor-count]').textContent = root.dataset.stepCount
            .replace('%current%', current + 1).replace('%total%', steps.length);
    ...
    const updateSummary = () => {
        ...
        root.querySelector('[data-advisor-summary]').innerHTML = selected.length
            ...
            : `<div><dt>${root.dataset.summaryEmpty}</dt><dd>${root.dataset.summaryHint}</dd></div>`;
    };
    ...
    form.addEventListener('submit', async (event) => {
        ...
        submitButton.textContent = root.dataset.loading;
        ...
        try {
            ...
            message.textContent = root.dataset.requestFailed;
