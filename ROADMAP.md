...
- **ID/Priorität/Status/Bereich:** P1-003 / P1 / Behoben in PR #121–#125 / CI
...
- **Umsetzung PR #122–#124:** PHPStan wurde von initial 441 Findings schrittweise auf 0 gebracht: PR #122 aktivierte die vorhandenen PHPStan-Extensions und behob einen echten Sylius-Calculator-Fehler, PR #123 bereinigte `src/` auf 0 und PR #124 bereinigte `tests/` sowie den vollständigen Repository-Lauf auf 0.
- **Umsetzung PR #125:** Die neu gemessenen 51 ECS-Findings in 51 Dateien unter dem bewusst beibehaltenen Application-Scope `src/` wurden mechanisch mit dem Sylius Labs Coding Standard bereinigt. ECS läuft nun mit `vendor/bin/ecs check --no-progress-bar` blockierend in den statischen CI-Checks.
- **Gate-Stand:** Composer Validate, Composer Audit, Plattformanforderungen, Twig-, YAML- und Container-Lint, PHPStan Level 9, Doctrine-Schemavalidierung sowie ECS/Sylius Coding Standard sind aktiv und blockierend.
...
- **Abschluss:** Keine PHPStan-Findings. Der reproduzierbare Level-9-Voll-Lauf ist grün; es wurde weder eine Baseline noch eine breite Ignore-Liste erzeugt.
