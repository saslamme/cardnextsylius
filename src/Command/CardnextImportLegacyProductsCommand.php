...
{
    public function __construct(private readonly CardnextLegacyProductImporter $importer)
    {
        parent::__construct();
    }
    ...
    {
        $this->addArgument('zip', InputArgument::REQUIRED)->addOption('dry-run', null, InputOption::VALUE_NONE)->addOption('report', null, InputOption::VALUE_REQUIRED, 'JSON report path', 'var/import/cardnext-legacy-import-report.json');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!is_string($zip) || !is_string($report)) {
            $io->error('ZIP and report paths must be strings.');

            return self::INVALID;
        }

        try {
            $r = $this->importer->import($zip, (bool) $input->getOption('dry-run'), $report);
        } catch (\Throwable $e) {
