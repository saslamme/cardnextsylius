...
final class LegacyDescriptionAttributeExtractor
{
    ...
    private function normalizePrinterTechnology(string $value): ?array
    {
        ...
        if (str_contains($fold, 'retransfer') || str_contains($fold, 'reversetransfer')) return ['retransfer'];
        if (str_contains($fold, 'direktkartendruck') || str_contains($fold, 'directtocard') || str_contains($fold, 'dyesublimation') || str_contains($fold, 'thermotransfer')) return ['direct_to_card'];
        return null;
    }
    ...
    private function normalizeResolution(string $value): ?array
    {
        ...
        if (preg_match('/(^|\D)600\s*dpi/i', $value)) return ['dpi_600'];
        if (preg_match('/(^|\D)300\s*dpi/i', $value)) return ['dpi_300'];
        return null;
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'beidseit') || str_contains($fold, 'duplex')) return ['duplex'];
        if (str_contains($fold, 'einseit') || str_contains($fold, 'simplex')) return ['single'];
        return null;
    }
    ...
    private function normalizePrintMode(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'farb') || str_contains($fold, 'color') || str_contains($fold, 'colour')) $result[] = 'color';
        if (str_contains($fold, 'monochrom') || str_contains($fold, 'schwarzweiss')) $result[] = 'monochrome';
        return $result;
    }
    ...
    private function normalizeCardFormat(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'cr80') || str_contains($fold, 'id1') || (str_contains($fold, '856') && str_contains($fold, '54'))) $result[] = 'cr80';
        if (str_contains($fold, 'cr79')) $result[] = 'cr79';
        return $result;
    }
    ...
    private function normalizeConnectivity(string $value, string $target): array
    {
        ...
        foreach ($map as $needle => $choice) if (str_contains($fold, $needle)) $result[] = $choice;
    ...
    private function normalizeEncoding(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'magnet')) $result[] = 'magstripe';
        if (str_contains($fold, 'kontaktchip') || str_contains($fold, 'contactchip') || str_contains($fold, 'smartcardkontakt')) $result[] = 'contact_chip';
        if (str_contains($fold, 'kontaktlos') || str_contains($fold, 'rfid') || str_contains($fold, 'nfc') || str_contains($fold, 'mifare')) $result[] = 'contactless';
        return array_values(array_unique($result));
    }
    ...
    private function normalizeFrequency(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, '125khz') || str_contains($fold, '1342khz') || str_contains($fold, 'lf')) $result[] = 'lf_125';
        if (str_contains($fold, '1356mhz') || str_contains($fold, 'hf') || str_contains($fold, 'nfc')) $result[] = 'hf_1356';
        if (str_contains($fold, '860960mhz') || str_contains($fold, '865') || str_contains($fold, '868') || str_contains($fold, '915') || str_contains($fold, 'uhf')) $result[] = 'uhf';
        return array_values(array_unique($result));
    }
    ...
    private function normalizeRfidTechnologies(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'mifareclassic') || str_contains($fold, 'mifareclassik')) $result[] = 'mifare_classic';
        if (str_contains($fold, 'desfire')) $result[] = 'mifare_desfire';
        if (str_contains($fold, 'ntag') || str_contains($fold, 'nfc')) $result[] = 'ntag';
        if (str_contains($fold, 'legic')) $result[] = 'legic';
        if (str_contains($fold, 'hidprox')) $result[] = 'hid_prox';
        if (str_contains($fold, 'iclass') || str_contains($fold, 'seos')) $result[] = 'hid_iclass';
        if (str_contains($fold, 'em4102') || str_contains($fold, 'em4200')) $result[] = 'em4102';
        if (str_contains($fold, 'felica')) $result[] = 'felica';
        return array_values(array_unique($result));
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'oem') || str_contains($fold, 'embedded')) return ['oem'];
        if (str_contains($fold, 'panel') || str_contains($fold, 'einbau')) return ['panel_mount'];
        if (str_contains($fold, 'wand') || str_contains($fold, 'aufbau') || str_contains($fold, 'surface')) return ['surface_mount'];
        if (str_contains($fold, 'desktop') || str_contains($fold, 'tisch')) return ['desktop'];
        return null;
    }
    ...
    private function normalizeOutputMode(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'tastatur') || str_contains($fold, 'keyboard')) $result[] = 'keyboard';
        if (str_contains($fold, 'virtualcom') || str_contains($fold, 'vcom')) $result[] = 'virtual_com';
        if (str_contains($fold, 'sdk') || str_contains($fold, 'api')) $result[] = 'sdk';
        if (str_contains($fold, 'wiegand') || str_contains($fold, 'osdp')) $result[] = 'wiegand_osdp';
        return array_values(array_unique($result));
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'polycarbonat') || str_contains($fold, 'polycarbonate')) return ['polycarbonate'];
        if (str_contains($fold, 'petg')) return ['petg'];
        if (str_contains($fold, 'pvc')) return ['pvc'];
        if (str_contains($fold, 'abs')) return ['abs'];
        if (str_contains($fold, 'papier') || str_contains($fold, 'karton')) return ['paper'];
        return null;
    }
    ...
    private function normalizeSurface(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'glanz') || str_contains($fold, 'gloss')) $result[] = 'glossy';
        if (str_contains($fold, 'matt')) $result[] = 'matte';
        if (str_contains($fold, 'signatur') || str_contains($fold, 'unterschrift')) $result[] = 'signature';
        return array_values(array_unique($result));
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'hico')) return ['hico'];
        if (str_contains($fold, 'loco')) return ['loco'];
        if (str_contains($fold, 'ohne') || str_contains($fold, 'kein')) return ['none'];
        return null;
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'hochformat') || str_contains($fold, 'portrait')) return ['portrait'];
        if (str_contains($fold, 'querformat') || str_contains($fold, 'landscape')) return ['landscape'];
        if (str_contains($fold, 'universal') || (str_contains($fold, 'hoch') && str_contains($fold, 'quer'))) return ['universal'];
        return null;
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'ymckok')) return ['ymckok'];
        if (str_contains($fold, 'ymcko')) return ['ymcko'];
        if (str_contains($fold, 'ymck')) return ['ymck'];
        if (str_contains($fold, 'retransfer') || str_contains($fold, 'film')) return ['retransfer'];
        if (str_contains($fold, 'overlay') || str_contains($fold, 'schutzfilm')) return ['overlay'];
        if (str_contains($fold, 'monochrom') || preg_match('/(^|\W)K(\W|$)/i', $value)) return ['monochrome'];
        return null;
    }
    ...
    private function normalizeBarcodeDimension(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, '1d')) $result[] = '1d';
        if (str_contains($fold, '2d') || str_contains($fold, 'qr')) $result[] = '2d';
        if (str_contains($fold, 'dpm')) $result[] = 'dpm';
        return array_values(array_unique($result));
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'areaimager') || str_contains($fold, 'areaimage')) return ['area_imager'];
        if (str_contains($fold, 'linearimager') || str_contains($fold, 'linearimage')) return ['linear_imager'];
        if (str_contains($fold, 'laser')) return ['laser'];
        return null;
    }
    ...
    private function normalizeIdentificationMethods(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'rfid') || str_contains($fold, 'karte')) $result[] = 'rfid';
        if (str_contains($fold, 'finger')) $result[] = 'fingerprint';
        if (str_contains($fold, 'gesicht') || str_contains($fold, 'face')) $result[] = 'face';
        if (str_contains($fold, 'pin') || str_contains($fold, 'tastatur')) $result[] = 'pin';
        if (str_contains($fold, 'barcode') || str_contains($fold, 'qr')) $result[] = 'barcode';
        return array_values(array_unique($result));
    }
    ...
    private function normalizeAccessProtocols(string $value): array
    {
        $fold = self::fold($value);
        $result = [];
        if (str_contains($fold, 'osdp')) $result[] = 'osdp';
        if (str_contains($fold, 'wiegand')) $result[] = 'wiegand';
        if (str_contains($fold, 'rs485')) $result[] = 'rs485';
        if (str_contains($fold, 'tcpip') || str_contains($fold, 'ethernet')) $result[] = 'tcpip';
        return array_values(array_unique($result));
    }
    ...
    {
        $fold = self::fold($value);
        if (str_contains($fold, 'controller')) return ['controller'];
        if (str_contains($fold, 'turmodul') || str_contains($fold, 'doormodule')) return ['door_module'];
        if (str_contains($fold, 'terminal')) return ['terminal'];
        if (str_contains($fold, 'leser') || str_contains($fold, 'reader')) return ['reader'];
        if (str_contains($fold, 'zubehor') || str_contains($fold, 'accessory')) return ['accessory'];
        return null;
    }
    ...
    private function normalizeBoolean(string $value): ?bool
    {
        $fold = self::fold($value);
        if ($fold === '') return null;
        if (preg_match('/\b(nein|no|false|ohne|nicht)\b/i', $value)) return false;
        if (preg_match('/\b(ja|yes|true|unterst[uü]tzt|integriert|vorhanden)\b/iu', $value)) return true;
        return null;
    }

    private function normalizeWarrantyMonths(string $value): ?int
    {
        if (!preg_match('/\d+/', $value, $m)) return null;
        $n = (int) $m[0];
        $fold = self::fold($value);
        if (str_contains($fold, 'jahr')) return $n * 12;
        return $n;
    }
