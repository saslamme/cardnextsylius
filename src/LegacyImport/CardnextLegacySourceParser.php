...
final readonly class CardnextLegacySourceParser
{
    ...
    public function parse(string $zipPath): LegacyImportPlan
    {
        ...
        foreach (['products/shop.artindex', 'products/shop.vendor', 'products/shop.attrib'] as $required) {
            if ($zip->locateName($required) === false) {
                throw new \RuntimeException("Central source file $required is missing.");
            }
        }
        $vendors = $this->lines($zip, 'products/shop.vendor');
        $vendorVocabulary = [];
        foreach ($vendors as $vendor) {
            $vendorVocabulary[self::normalize($vendor)] = $vendor;
        }
        $artIndex = [];
        foreach ($this->lines($zip, 'products/shop.artindex') as $line) {
            [$id, $mpn] = array_pad(explode('|', $line), 2, '');
            if (trim($id) !== '' && trim($mpn) !== '') {
                $artIndex[trim($id)] = trim($mpn);
            }
        }
        $attributeVocabulary = [];
        foreach ($this->lines($zip, 'products/shop.attrib') as $line) {
            ...
            if ($name !== '') {
                $attributeVocabulary[LegacyAttributeMapper::fold($name)] = true;
            }
        }
        ...
        for ($i = 0; $i < $zip->numFiles; ++$i) {
            ...
            foreach (preg_split('/\R/', $content) ?: [] as $line) {
                ...
                if (count($fields) !== 62) {
                    throw new \RuntimeException(sprintf('%s has an unsupported field count.', $name));
                }
    ...
    private function record(string $file, array $f, array $vendors, array $artIndex, array $attributeVocabulary): LegacyProductRecord
    {
        ...
        foreach (explode(':#', $f[25]) as $item) {
            ...
            if (count($parts) >= 2 && trim($parts[0]) !== '') {
                $legacy[trim($parts[0])] = trim($parts[1]);
            }
        }
        $mapped = ($this->attributeMapper ?? new LegacyAttributeMapper())->map($legacy, $attributeVocabulary);
        $id = trim($f[0]);
        $datMpn = trim($f[3]);
        $indexedMpn = $artIndex[$id] ?? '';
        $reasons = [];
        if ($indexedMpn !== '' && $datMpn !== '' && self::normalize($indexedMpn) !== self::normalize($datMpn)) {
            $reasons[] = 'artindex_mpn_mismatch';
        }
        ...
        if ($manufacturer === '' || !isset($vendors[self::normalize($manufacturer)])) {
            $reasons[] = $manufacturer === '' ? 'missing_manufacturer' : 'unknown_manufacturer';
        }
        if ($mapped['unknown'] !== []) {
    ...
    private function deduplicate(array $raw, int $zipFiles, int $sourceFiles, int $ignored, int $vendorCount, int $artIndexCount, int $attributeVocabularyCount): LegacyImportPlan
    {
        ...
        foreach ($raw as $r) {
            if ($r->manufacturer !== '') {
                $manufacturersByMpn[self::normalize($r->manufacturerPartNumber)][self::normalize($r->manufacturer)] = $r->manufacturer;
            }
        }
        foreach ($raw as $i => $r) {
        ...
        foreach ($raw as $record) {
            ...
            $manufacturer = self::normalize($record->manufacturer);
            ...
            $old = $byKey[$key];
            $reasons = array_values(array_unique([...$old->reviewReasons, ...$record->reviewReasons]));
            if ($old->price !== null && $record->price !== null && $old->price !== $record->price) {
                $conflicts[$key] = 'price_conflict';
                $reasons[] = 'price_conflict';
                $conflictDetails[$key] = ['type' => 'price_conflict', 'manufacturer' => $old->manufacturer, 'mpn' => $old->manufacturerPartNumber, 'legacy_ids' => [$old->legacyId, $record->legacyId], 'prices' => [$old->price, $record->price]];
            }
            if ($old->gtin !== null && $record->gtin !== null && $old->gtin !== $record->gtin) {
                $conflicts[$key] = 'gtin_conflict';
        ...
        foreach ($records as $r) {
            $mpnToCode[self::normalize($r->manufacturerPartNumber)][] = self::productCode($r);
        }
        foreach ($records as $i => $r) {
            ...
            foreach ($r->compatibilityReferences as $ref) {
                ...
                if (count($targets) === 1 && $targets[0] !== self::productCode($r)) {
                    $related[] = $targets[0];
                } elseif ($targets === []) {
                    $reasons[] = 'unresolved_relation';
                }
            }
        ...
        foreach ($records as $r) {
            if ($r->manufacturer !== '') {
                $manufacturers[self::normalize($r->manufacturer)] = true;
            }
        }
    ...
    public static function productCode(LegacyProductRecord $r): string
    {
        return 'LEGACY_' . self::normalize($r->manufacturer) . '_' . self::normalize($r->manufacturerPartNumber ?: $r->legacyId);
    }

    public static function normalize(string $value): string
    {
        return mb_strtoupper(preg_replace('/[^A-Z0-9]+/i', '', trim($value)) ?? '');
    }

    /** @return list<string> */
    private function lines(\ZipArchive $zip, string $name): array
    {
        ...
        return array_values(array_filter(array_map(fn ($v) => trim($this->utf8($v)), preg_split('/\R/', $s) ?: []), fn ($v) => $v !== ''));
    }
    ...
    {
        $html = preg_replace('#<(script|iframe)\b[^>]*>.*?</\1>#is', '', $html) ?? '';

        return preg_replace('/\s(on\w+|style)=("[^"]*"|\'[^\']*\')/i', '', $html) ?? '';
    }
}
