...
class Configurator
{
    ...
    public function __construct(string $code, string $name) { $this->code=$code; $this->name=$name; $this->createdAt=$this->updatedAt=new \DateTimeImmutable(); $this->sections=new ArrayCollection(); }
    ...
    /** @return Collection<int, ConfiguratorSection> */ public function getSections(): Collection { return $this->sections; }
    public function addSection(ConfiguratorSection $section): void { if (!$this->sections->contains($section)) {$this->sections->add($section); $section->setConfigurator($this);} }
