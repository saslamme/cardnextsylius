...
final readonly class QuoteOrderConverter
{
    ...
    {
        if ($quote->getOrder() !== null) return $quote->getOrder();
        ...
        return $this->entityManager->wrapInTransaction(function () use ($quote, $admin): Order {
            if ($quote->getId() !== null) {
                ...
                if (!$locked instanceof Quote) throw new \DomainException('Das Angebot wurde nicht gefunden.');
                $quote = $locked;
            }
            if ($quote->getOrder() !== null) return $quote->getOrder();
            $this->validate($quote);
            ...
            if (!$channel instanceof Channel) throw new \DomainException('Der im Angebot gespeicherte Channel existiert nicht.');
            ...
            $order->setChannel($channel); $order->setLocaleCode($quote->getLocaleCode()); $order->setCurrencyCode($quote->getCurrencyCode());
            $order->setCustomer($customer); $order->setBillingAddress($this->address($quote)); $order->setShippingAddress($this->address($quote));

            foreach ($quote->getItems() as $quoteItem) $this->addQuoteItem($order, $quoteItem, $quote);
            ...
            $order->completeCheckout(); $order->setCheckoutState(OrderCheckoutStates::STATE_COMPLETED); $order->setState(OrderInterface::STATE_NEW);
            ...
            if ($request->getStatus() !== QuoteRequestStatus::Closed && $request->getStatus()->canTransitionTo(QuoteRequestStatus::Closed)) $request->setStatus(QuoteRequestStatus::Closed);
    ...
    private function address(Quote $quote): Address
    {
        ...
        [$firstName, $lastName] = self::splitContactName($quote->getCustomerContactName());
        $address->setFirstName($firstName); $address->setLastName($lastName); $address->setCompany($quote->getCustomerCompany() ?: null);
        $address->setStreet(trim(($quote->getCustomerStreet() ?? '').' '.($quote->getCustomerHouseNumber() ?? '')));
        $address->setPostcode($quote->getCustomerPostalCode()); $address->setCity($quote->getCustomerCity());
        $address->setCountryCode($quote->getCustomerCountryCode() === null ? null : strtoupper($quote->getCustomerCountryCode())); $address->setPhoneNumber($quote->getCustomerPhone());
        return $address;
    }
    ...
    private function addAdjustment(object $adjustable, string $type, string $label, int $amount, array $details = []): void
    {
        ...
        $adjustment->setType($type); $adjustment->setLabel($label); $adjustment->setAmount($amount); $adjustment->setDetails($details);
        $adjustable->addAdjustment($adjustment); $adjustment->lock();
    }
