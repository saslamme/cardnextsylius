...
final class QuoteRevisionFactory
{
    ...
    {
        if (!in_array($source->getStatus(), [QuoteStatus::Ready, QuoteStatus::Sent, QuoteStatus::Rejected], true)) throw new \DomainException('Aus diesem Angebot kann keine neue Version erstellt werden.');
        ...
        $this->calculator->calculate($q); $source->transitionTo(QuoteStatus::Superseded); $source->getQuoteRequest()->addHistory(new \App\Entity\Quote\QuoteRequestHistory('quote_superseded',null,null,'Angebot '.$source->getNumber().' v'.$source->getVersion().' ersetzt')); $request=$source->getQuoteRequest(); if ($request->getStatus() === \App\Enum\Quote\QuoteRequestStatus::Question) $request->setStatus(\App\Enum\Quote\QuoteRequestStatus::InProgress); $this->em->persist($q); return $q;
    }
}
