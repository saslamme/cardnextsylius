...
{ /** @param iterable<ConfiguratorPriceRule> $rules @return list<ConfiguratorPriceRule> */ public function resolve(iterable $rules, int $quantity, string $channelCode, string $currencyCode): array
{
    $dimensions = [];
    foreach ($rules as $r) {
    ...
    }usort($resolved, fn ($a, $b) => [$a->getPriceType()->value,-$a->getPriority(),$a->getChargeCode()] <=> [$b->getPriceType()->value,-$b->getPriority(),$b->getChargeCode()]);
    return $resolved;
}
}
