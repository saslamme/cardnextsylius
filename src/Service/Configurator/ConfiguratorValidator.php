...
{ /** @param iterable<ConfiguratorDependency> $dependencies */ public function validate(ConfiguratorConfiguration $cfg, Configurator $model, iterable $dependencies = []): ValidationResult
{
    ...
    /** @param list<ValidationError> $errors */ private function validateSelection(ConfiguratorField $field, mixed $selection, array &$errors): void
    {
        ...
        if (in_array($type, [FieldType::INTEGER, FieldType::QUANTITY], true)) {
            if (!$this->isCanonicalInteger($selection)) {
    ...
    private function isCanonicalInteger(mixed $value): bool
    {
        return is_int($value) || (is_string($value) && preg_match('/^-?(?:0|[1-9][0-9]*)$/D', $value) === 1);
    }
