...
use App\Dto\Configurator\{ConfiguratorConfiguration,ValidationError,ValidationResult};
use App\Entity\Configurator\{Configurator,ConfiguratorDependency,ConfiguratorField};
use App\Enum\Configurator\{DependencyEffect,DependencyOperator,FieldType};

final class ConfiguratorValidator
{ /** @param iterable<ConfiguratorDependency> $dependencies */ public function validate(ConfiguratorConfiguration $cfg, Configurator $model, iterable $dependencies = []): ValidationResult
{
    $errors = [];
    ...
    /** @param list<ValidationError> $errors */ private function validateSelection(ConfiguratorField $field, mixed $selection, array &$errors): void
    {
        $type = $field->getType();
        $codes = $type === FieldType::MULTIPLE_CHOICE ? (is_array($selection) ? $selection : [$selection]) : (is_array($selection) ? $selection : [$selection]);
        if ($type === FieldType::SINGLE_CHOICE && is_array($selection) && count($selection) !== 1) {
            $errors[] = new ValidationError($field->getCode(), 'single_choice_count', 'Single choice accepts exactly one value.');
            ...
            return;
        }if (in_array($type, [FieldType::INTEGER,FieldType::DECIMAL,FieldType::QUANTITY], true)) {
            ...
            }if ($step !== null && $min !== null && abs(fmod($v - (float)$min, (float)$step)) > 1e-9) {
    ...
    /** @param array<string,mixed> $s */ private function matches(ConfiguratorDependency $d, array $s): bool
    {
        $actual = $s[$d->getSourceField()->getCode()] ?? null;
