...
final class ConfigurationHashGenerator
{
    ...
    {
        $data = $configuration->jsonSerialize();
        $data['currencyCode'] = strtoupper($configuration->currencyCode);

        return hash('sha256', json_encode($this->canonicalize($data), \JSON_THROW_ON_ERROR | \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE));
    }
