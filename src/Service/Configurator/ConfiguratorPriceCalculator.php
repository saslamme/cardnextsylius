...
final readonly class ConfiguratorPriceCalculator
{
    ...
    public function calculate(ConfiguratorConfiguration $configuration, ChannelInterface $channel, string $currencyCode): ConfiguratorPriceResult
    {
        ...
        return $this->calculateRules($configuration, $model, $values, $candidates, $channel->getCode(), $currencyCode);
    }

    /** @param list<ConfiguratorValue> $values @param iterable<ConfiguratorPriceRule> $rules */
    public function calculateRules(ConfiguratorConfiguration $cfg, Configurator $model, array $values, iterable $rules, string $channelCode, string $currency): ConfiguratorPriceResult
    {
        ...
        foreach ($resolved as $rule) {
            ...
            if ($rule->getPriceType() === PriceType::UNIT) {
                // Quantity is applied once by unitTotal. QUANTITY explicitly means the same
                // per-unit behavior as NONE and must never square the order quantity.
                ...
                $lines[] = $this->line($rule, $factor * $cfg->quantity, $effectiveUnit * $cfg->quantity);
            } else {
        ...
        foreach ($deferred as $rule) {
            ...
            // PERCENT applies basis points to its selected monetary base, then applies
            // NONE (once), QUANTITY (per ordered item), or FIELD_VALUE explicitly.
            $factor = $this->factor($rule, $cfg);
